...
def test_caching():
    ...
    response = requests.post(f"{BASE_URL}/api/users", 
                           json=user_data, headers=AUTH_HEADER)
    ...
    print(f"DB Response ({db_time:.3f}s): from_cache={db_response['data']['from_cache']}")
    ...
    print(f"Cache Response ({cache_time:.3f}s): from_cache={cache_response['data']['from_cache']}")
